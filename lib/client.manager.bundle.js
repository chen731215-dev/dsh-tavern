  ...
  factory: (require) => {
    ...
    function panelHTML() {
      return [
        ...
        '      <button id="tavern-preset-batch" type="button" class="t-btn-secondary" style="white-space:nowrap">📋 批量</button>',
        '    </div>',
    ...
    function mountTavernManager(root) {
      ...
      function handleWbFile(file) {
        ...
        return file.text().then(function (text) {
          ...
          if (typeof wbEntries !== 'undefined') {
            entries.forEach(function (e) { wbEntries.push(e); });
            // 添加到分组
            var existingGroup = wbGroups.find(function (g) { return g.name === name; });
            if (existingGroup) {
              entries.forEach(function (e) { existingGroup.entries.push(e); });
            } else {
              wbGroups.push({ name: name, entries: entries.slice(), enabled: true });
            }
            renderWbList();
            // 保存到 API
            var sid = getCurrentSessionId();
        ...
        renderGroups.forEach(function (group, gIdx) {
          ...
          if (groupExpanded) {
            ...
            group.entries.forEach(function (entry, eIdx) {
              ...
              html += '<button data-wb-idx="' + flatIdx + '" data-wb-action="delete" onclick="event.stopPropagation()" style="padding:2px 6px;border-radius:3px;border:none;background:#e74c3c;color:#fff;cursor:pointer;font-size:10px;flex-shrink:0">删</button>';
              html += '</div>';
              // 展开内容
              if (expanded) {
      ...
      function saveWb() {
        var sid = getCurrentSessionId();
        fetch('/api/tavern/worldbook', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ entries: wbEntries, injectMode: wbMode, sessionId: sid }) }).then(function (r) { return r.json(); }).then(function (data) {
      ...
      function renderRelationsGraph(relations) {
        ...
        svg.style.cssText = 'background:rgba(0,0,0,0.2);border-radius:8px;border:1px solid rgba(255,255,255,0.1);cursor:grab;';
        // 定义箭头
        var defs = document.createElementNS('http://www.w3.org/2000/svg', 'defs');
        defs.innerHTML = '<marker id="arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="rgba(122,184,255,0.6)"/></marker>';
        ...
        edges.forEach(function (e) {
          ...
          line.setAttribute('stroke', 'rgba(122,184,255,0.4)');
        ...
        nodes.forEach(function (n) {
          ...
          circle.setAttribute('fill', 'rgba(122,184,255,0.15)');
          circle.setAttribute('stroke', '#7ab8ff');
          ...
          text.textContent = (n.label || n.id).slice(0, 4);
          ...
          window.addEventListener('mousemove', function (ev) {
            if (!dragging) return;
            var pt = svgPoint(svg, ev);
            pos.x = pt.x - offset.x; pos.y = pt.y - offset.y;
            circle.setAttribute('cx', pos.x); circle.setAttribute('cy', pos.y);
            text.setAttribute('x', pos.x); text.setAttribute('y', pos.y + 4);
            // 更新连线
            edgeLayer.innerHTML = '';
            edges.forEach(function (e2) {
              ...
              line2.setAttribute('stroke', 'rgba(122,184,255,0.4)');
              ...
              if (label2) {
                ...
                var tw2 = label2.length * 12 + 12;
                bg2.setAttribute('x', mx2 - tw2 / 2); bg2.setAttribute('y', my2 - 10);
                bg2.setAttribute('width', tw2); bg2.setAttribute('height', 20);
                bg2.setAttribute('rx', 10); bg2.setAttribute('fill', 'rgba(30,30,46,0.9)');
                bg2.setAttribute('stroke', 'rgba(122,184,255,0.3)');
                edgeLayer.appendChild(bg2);
                ...
                txt2.setAttribute('fill', '#7ab8ff'); txt2.setAttribute('font-size', '11');
                txt2.textContent = label2;
                edgeLayer.appendChild(txt2);
              }
            });
          });
          window.addEventListener('mouseup', function () { dragging = false; gnode.style.cursor = 'grab'; });
        ...
        info.style.cssText = 'margin-bottom:6px;font-size:12px;color:var(--dsw-alias-label-secondary)';
        info.innerHTML = '<strong style="color:#7ab8ff">' + nodes.length + '</strong> 个角色，<strong style="color:#7ab8ff">' + edges.length + '</strong> 条关系（拖动节点可调整位置）';
        g.appendChild(info);
        g.appendChild(svg);
      }
      ...
      container.querySelector('#tavern-preset-new').addEventListener('click', function () {
        // 用会话标题作为默认预设名
        var defaultName = getSessionTitleFromDOM() || '新预设';
        var name = prompt('新预设名称：', defaultName);
      ...
      container.querySelector('#tavern-preset-batch').addEventListener('click', function () {
          ...
          presetList.forEach(function (p) {
            ...
            item.addEventListener('click', function (e) {
              if (e.target.classList.contains('del-btn')) {
                e.stopPropagation();
                if (confirm('确定删除预设「' + (p.name || '') + '」？删除后无法恢复。')) {
