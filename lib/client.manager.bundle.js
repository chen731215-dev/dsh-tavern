    ...
    function createPanel() {
      ...
      container.innerHTML = [
        ...
        '  <div class="card"><strong>预设</strong>（支持 JSON，可导入多份并切换）<div id="tavern-preset-list" style="margin-top:6px"></div><div style="margin-top:4px"><input type="file" id="tavern-preset-file" accept=".json,application/json"> <button id="tavern-insert-foot" type="button">插入足部描写</button></div></div>',
          ...
          '  <div class="card"><strong>🧠 记忆</strong>（自动总结/手动保存记忆，会注入到 Harness 预设）<textarea id="tavern-memory-text" rows="6" style="width:100%;box-sizing:border-box;margin-top:6px" placeholder="当前记忆内容..."></textarea><div style="margin-top:4px"><button id="tavern-memory-save" type="button">保存记忆</button> <button id="tavern-memory-load" type="button">读取记忆</button></div></div>',
